...
- **自定义工具 SDK**：通过 `code-agent tool new` CLI 命令快速创建和注册自定义工具，支持完整的工具生命周期开发
...
- **线程共享**：通过 `POST /threads/:id/share` 和 `DELETE /threads/:id/share` 端点分享或取消分享线程，支持 Read / Edit / Admin 三种 SharePermission 权限级别
- **审计追踪**：`audit_log` 表全量记录协作操作（分享、修改、撤销），确保团队操作可追溯
- **共享线程发现**：通过 `GET /shared-threads` 端点查看当前用户可访问的所有共享线程
...
- 团队协作安全：SharePermission 三级权限控制（Read / Edit / Admin），线程共享端点认证与授权
- 审计追踪：audit_log 全量记录协作操作，防止越权访问他人线程
...
### 生态扩展
- [x] 插件系统（Hooks / Commands 扩展体系）
- [x] 自定义工具 SDK（`code-agent tool new` CLI 脚手架，见 [自定义工具SDK.md](doc/自定义工具SDK.md)）
- [x] 更多 LLM Provider（Anthropic / Google Gemini）— 已实现
- [x] Web UI 管理面板（React + TypeScript + Vite，线程管理/实时对话/Token 监控）
- [x] 团队协作功能（线程共享 + 审计追踪 + SharePermission 权限管理 + Team CRUD + 8 个 REST 端点）

### 架构集成
- [x] Delegator/Coder 双角色架构（基于 SubAgentManager）
- [x] Plan 引擎（三级分解 + DAG 依赖图）
- [x] 上下文增强（LLM 摘要 + 滑动窗口 + Token 预算）
- [x] Spec 驱动流水线 + 质量门禁（QualityGate 集成于 Session 生命周期）
- [x] 知识体系（Rules / Skills / KnowledgeProvider 集成于 Prompt 构建管道）
- [x] Command 扩展（PreCommand / PostCommand 钩子 + Plugin collect_commands()）
- [x] 数据飞轮闭环
