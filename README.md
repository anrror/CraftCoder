...
LLM 模型接入采用 Provider 模式。`ModelClient` trait 定义了统一的 `complete_stream` / `complete` 接口，不同模型服务实现各自的 Provider。新增一个模型服务只需实现该 trait，无需修改任何业务代码。当前支持 OpenAI 兼容 API，未来可扩展 Anthropic、Google Gemini 等原生协议。
...
otlp_endpoint = ""   # 需启用 otlp feature，留空则禁用
```

### Docker 部署

Rust 服务的 Docker 容器化部署计划在 v0.3 中实现。当前仅 Python 评估套件提供 Docker 支持：
