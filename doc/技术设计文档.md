...
> **Agent运行时**: Rust 9-Crate 工作区（protocol/core/tools/codex/eval/eval-runner/cli/app-server/code-agent-web）  
...
**在范围内**：Rust核心Agent引擎（ReAct循环、会话管理、工具路由）、多模型家族全链路集成、SQLite持久化数据层、CLI（TUI+Headless）/App Server/Web API三端、代码理解引擎（AST索引+混合检索）、工具集成（Shell/LSP/Git/MCP）、评估框架。
...
| **core** | `code-agent-core` | Agent核心引擎（ReActLoop, SessionManager, ContextManager, ToolRouter） | protocol, tools, codex |
...
| **code-agent-web** | `code-agent-web` | Web API 服务（Axum REST + SSE） | core, protocol |
