...
| **core** | `code-agent-core` | Agent核心引擎（ReActLoop, SessionManager, ContextManager, ToolRouter, ThreadManager）<br/>**扩展模块**：<br/>• `agent::plan` — Plan 引擎（TaskPlan, PlanNode DAG, Kahn 拓扑排序, PlanExecutor, QualityGate, KnowledgeProvider, PlanStore，8 文件）<br/>• `context` — 5 层压缩管道（BudgetReduction → Snip → Microcompact → ContextCollapse → AutoCompact）+ LLM 摘要器 + TokenBudgetAllocator<br/>• `tools::hook` — 工具生命周期钩子（ToolHook, HookRegistry, 8 种事件类型）<br/>• `tools::plugin` — 插件系统（Plugin, PluginManager, McpConnector, collect_commands）<br/>• `tools::command` — 命令系统（CommandRegistry, /help, /status, Command hooks）<br/>• `flywheel` — 数据飞轮（FlywheelCollector, FailureAnalyzer, ImprovementSuggester, NightlyPipeline）<br/>• `team::sharing` — 团队协作（ThreadManager::share_thread/check_access, SharePermission 权限模型）<br/>• `audit` — 审计追踪（AuditEvent, record_audit_event, audit_log 持久化表） | protocol, tools, codex |
...
CraftCoder 支持线程共享与团队协作，协作场景下的安全机制独立于单用户安全层，确保共享线程的数据访问受控、操作可追溯。

**SharePermission 权限模型**：

共享线程的访问权限通过 `SharePermission` 枚举精细控制：

| 权限级别 | 说明 | 允许操作 |
|----------|------|----------|
| **Read** | 只读查看 | 查看线程消息历史、状态信息 |
| **Comment** | 只读 + 评论 | Read 权限 + 添加评论消息 |
| **Edit** | 读写编辑 | Comment 权限 + 提交 Turn、执行工具调用 |

默认共享权限为 `Read`，由共享发起方在创建共享链接时指定权限级别。

**共享线程访问控制**：

```
请求 → AuthMiddleware（API Key 验证）
     → ShareAccessGuard（共享权限检查）
        ├─ 线程所有者：完全访问
        ├─ 共享接收者：按 SharePermission 级别限制
        └─ 无权限用户：403 Forbidden
     → ThreadManager::check_access(thread_id, user_id)
        → 是所有者？ → 直接放行
        → 有共享记录？ → 读取 SharePermission 级别 → 按级别授权
        → 无共享记录？ → 拒绝访问
```

**审计追踪（record_audit_event）**：

所有团队协作操作均记录到 `audit_log` 表，形成不可篡改的审计轨迹：

```sql
CREATE TABLE audit_log (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    event_type      TEXT NOT NULL,           -- 'thread_shared', 'thread_unshared', 'access_granted', 'access_denied', 'turn_submitted'
    thread_id       VARCHAR(64) NOT NULL,
    actor_user_id   VARCHAR(128) NOT NULL,   -- 操作发起者
    target_user_id  VARCHAR(128),            -- 操作目标（共享接收者等）
    permission      TEXT,                    -- SharePermission 级别（如适用）
    details         TEXT,                    -- JSON 格式补充信息
    created_at      TEXT NOT NULL DEFAULT (datetime('now')),
...
审计事件覆盖以下场景：
- `thread_shared` — 线程被共享给其他用户（记录共享目标 + 权限级别）
- `thread_unshared` — 共享被撤销
- `access_granted` — 共享接收者成功访问线程
- `access_denied` — 未授权用户尝试访问被拒绝
- `turn_submitted` — 共享接收者提交 Turn（记录提交者身份）

`record_audit_event(event_type, thread_id, actor, target, permission, details)` 在所有共享操作入口处被同步调用，确保"先审计、后执行"的不可否认性。
...
### 8.5 团队协作 API（code-agent-web）

基于 Web API 的线程共享与协作接口，允许用户将线程分享给团队成员并在权限范围内进行协作。

| 方法 | 路径 | 说明 | 认证 |
|------|------|------|------|
| POST | `/threads/:id/share` | 分享线程给指定用户 | 必需（API Key） |
| DELETE | `/threads/:id/share` | 撤销线程共享 | 必需（API Key） |
| GET | `/shared-threads` | 列出当前用户被分享的所有线程 | 必需（API Key） |

**POST /threads/:id/share** — 创建共享：

```json
// 请求
{
    "target_user_id": "developer-001",
    "permission": "read"  // "read" | "comment" | "edit"
}

// 响应 201
{
    "share_id": "shr_abc123",
    "thread_id": "ses_def456",
    "target_user_id": "developer-001",
    "permission": "read",
...
- 仅线程所有者可创建共享
- 自动记录 `thread_shared` 审计事件（actor=所有者, target=接收者, permission 级别）
- 同一线程对同一用户不允许重复共享（返回 409 Conflict）

**DELETE /threads/:id/share** — 撤销共享：
...
- 仅线程所有者可撤销共享
- 自动记录 `thread_unshared` 审计事件
- 撤销后目标用户无法再访问该线程

**GET /shared-threads** — 列出被分享的线程：
...
// 响应 200
{
    "shared_threads": [
        {
            "thread_id": "ses_def456",
            "owner_user_id": "tech-lead",
            "permission": "read",
            "shared_at": "2026-07-28T10:30:00Z",
            "thread_title": "修复用户模块认证Bug"
        }
    ]
}
```
...
```
GET/POST /threads/:id/* 请求
    → 认证中间件（API Key → user_id）
    → 线程所有者检查：owner_user_id == user_id？
        → 是：完全访问
        → 否：查询 share 表
            → 存在活跃共享 → 校验 permission 级别
            → 无共享 → 403 Forbidden
    → 每次授权访问记录 audit_log（access_granted）
    → 每次拒绝记录 audit_log（access_denied）
```
...
| | Custom Tool SDK | `code-agent-tool new` 脚手架 CLI + 2 个示例工具 + 开发者文档 | 第三方工具扩展开发 |
