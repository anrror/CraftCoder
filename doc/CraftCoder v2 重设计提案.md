...
> **状态**: 全部实现 — 架构参考文档  
> **版本**: v2（对应代码库 `code-agent-rs` 当前状态，599 测试全量通过）  
> **用途**: 本文档记录 CraftCoder 的系统架构设计、核心决策依据和各子系统实现方案。所有描述的 Phase（A-G）及集成阶段（H）均已完成开发并落地为生产代码。本文档作为新开发者理解系统架构的权威参考，也作为未来迭代的设计基线。
>
...
> ⚠️ **重要说明**：在起草架构设计之初，对 `code-agent-rs` 现有代码库进行了深度审查（Phase A0）。发现设计中部分被描述为"v2 新增"的功能实际上已在代码库中以独立组件的形式存在。本节如实记录现有状态，为后续实施路线的修正提供依据。
...
| **任务处理** | 无结构化规划，直接 ReAct 推理 | 三级分解 + DAG 依赖图 | 引入 Plan 模式 |
...
| **知识** | 环境变量 + 扁平 TOML 配置（11 字段） | Rules + Skills + 企业知识库（节式配置） | 系统化的知识管理 |
| **反馈** | Flywheel（ErrorTrace 聚类）+ FeedbackCollector（SQLite 评分）+ Eval（基准测试）三个孤岛 | 数据飞轮闭环（反馈→飞轮→评估→自动更新→调度） | 集成现有三个系统 |
| **工具生态** | Tool trait + ToolRegistry + MCP 协议 + LSP 集成 | MCP + 插件 + Hooks + Commands 四层生态 | 扩展性大幅提升 |
...
| **SubAgentRunner 生产实现** | 新建 `SessionRunner` 实现 `SubAgentRunner` trait，内部创建 `Session` + 执行 ReAct 循环（替代了原有的 `MockRunner`） |
| **Retry/Replan 策略** | 在 `SubAgentManagerImpl` 的 spawn→wait→error 流程中添加 RetryGate / ReplanGate |
| **Session 使用 SubAgentManager** | Delegator 中编排 Session，`Session::run_turn()` 通过 SubAgentManager 执行 |
...
| 维度 | 现有 Compactor 5 层 | 设计架构 L1-L5 |
|------|--------------------|-------------|
...
两套压缩机制可以共存且互补：
1. **架构 L1（Token 预算）** 在 Delegator/Coder 创建时分配 Token 配额
2. **现有 Compactor** 在每个 Coder/Session 内部独立运行，进行渐进式压缩
3. **架构 L2-L4** 是对现有 Compactor 层的增强（滑动窗口、LLM 摘要、AST 裁剪）
4. **架构 L5（EPSS）** 使用 SubAgentManager 已提供的隔离机制
...
graph LR
    ...
    style FW fill:#abebc6
    style FB fill:#abebc6
    style EV fill:#abebc6
```
...
| # | 缺口 | 解决方案 | 状态 |
|---|------|------|:---:|
| 1 | **自动 trace 捕获** | `tools/` 和 `agent/` 中添加 `ErrorTrace` 创建逻辑 | ✅ |
| 2 | **反馈→飞轮** | `FeedbackCollector` 与 `FailureAnalyzer` 建立交叉引用管道 | ✅ |
| 3 | **飞轮→自动更新** | `NightlyPipeline::apply_suggestions()` 已接入 ToolRegistry | ✅ |
| 4 | **评估→飞轮** | `EvalResult` 被 Flywheel 消费，基准回归触发优化 | ✅ |
| 5 | **A/B 测试** | 并排比较框架已实现 | ✅ |
| 6 | **持续调度** | 调度器支持持续运行（非一次性批处理） | ✅ |
| 7 | **统一存储** | ErrorTrace + FeedbackEvent + EvalResult 统一关联分析 | ✅ |
| 8 | **ML 聚类** | FailureAnalyzer 已升级支持语义相似聚类 | ✅ |
...
> **全部完成**：v2 架构所有计划阶段（A-G）及集成阶段（H）已全部实现为生产代码。SubAgentManager、Plan Engine（TaskPlan/Decomposer/Kahn DAG/PlanExecutor/QualityGate/KnowledgeProvider/PlanStore，共 8 个文件）、5 层上下文压缩 + LLM Summarizer + TokenBudgetAllocator、QualityGate + SpecGate 流水线、KnowledgeProvider + KnowledgeStore + Rule 系统、Hook + Plugin + Command 生态、FlywheelCollector + FailureAnalyzer + ImprovementSuggester + NightlyPipeline 均已落地。全部 599 个测试通过。

### 12.1 已完成阶段一览

| 阶段 | 状态 | 性质 | 核心变更 | 影响 Crate |
|------|------|------|----------|-----------|
| **A0: 架构对齐** | ✅ 已完成 | 审查 | 审查现有基础设施，建立保留/重构/替换决策矩阵 | 全 crate |
| **A: Delegator/Coder 精化** | ✅ 已完成 | 重构+增强 | SubAgentManager + Delegator（`core/src/agent/sub_agent/`），SessionRunner，CoderSpec，RetryGate/ReplanGate | `core`, `protocol` |
| **B: Plan 引擎** | ✅ 已完成 | 新建 | TaskPlan + Decomposer + Kahn DAG 调度 + PlanExecutor + QualityGate + KnowledgeProvider + PlanStore（8 文件） | `core`, `protocol` |
| **C: 上下文增强** | ✅ 已完成 | 增强 | 5 层压缩 + LLM Summarizer + TokenBudgetAllocator（`core/src/context/`） | `core`, `codex` |
| **D: Spec 流水线** | ✅ 已完成 | 新建 | QualityGate + SpecGate 流水线，8 阶段编排，质量门禁评分 | `core`, `protocol` |
| **E: 知识体系** | ✅ 已完成 | 新建 | KnowledgeProvider + KnowledgeStore + Rule 系统（`core/src/agent/plan/knowledge.rs`） | `core`, `cli` |
| **F: 质量门禁** | ✅ 已完成 | 新建 | AI Code Review 引擎（逻辑+安全+性能+架构 4 维），OWASP 规则集，测试生成，变更覆盖率 | `core`, `tools` |
| **G: 工具生态** | ✅ 已完成 | 新建 | Hook 生命周期 + Plugin trait + Command 注册表（`core/src/tools/hook.rs`, `plugin.rs`, `command.rs`） | `core`, `tools`, `protocol` |
| **H: 数据飞轮集成** | ✅ 已完成 | 集成 | FlywheelCollector + FailureAnalyzer + ImprovementSuggester + NightlyPipeline，已集成现有 eval/metrics 基础设施 | `core`, `eval`, `cli` |
