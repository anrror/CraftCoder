...
### 实际开发进度（对应 README 路线图）

当前项目按以下版本节奏推进，与上述产品路线图（Phase 1-5）并行：

| 版本 | 状态 | 核心交付 |
|------|------|----------|
| **v0.1 — 基础框架** | ✅ 已完成 | Rust Cargo Workspace、Protocol 数据模型、Core LLM Provider、Tools 工具集、CLI TUI、Codex 代码理解引擎、Eval 评估框架、CI/CD 流水线 |
| **v0.2 — 交互完善** | ✅ 已完成 | Web API 服务（Axum REST + SSE）、VS Code 扩展（JSON-RPC 2.0）、MCP 协议支持、LSP 集成、配置管理、TUI 接入真实 Agent 引擎 |
| **v0.3 — 生产就绪** | 🔄 进行中 | OpenTelemetry 可观测性、Docker 容器化部署、多用户会话管理、权限与安全审计、性能优化与缓存 |
| **v0.4 — 生态扩展** | 📋 规划中 | 插件系统、自定义工具 SDK、更多 LLM Provider、Web UI 管理面板、团队协作功能 |
