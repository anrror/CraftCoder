...
|------|-----------|-----------------|------|
| `agent` | Agent 核心上下文 | `Session`, `ThreadManager`, `ReActLoop` | ReAct 推理循环、会话生命周期、线程管理（含团队协作：共享/审计）、多智能体协作 |
| `context` | 上下文管理上下文 | `ContextCompactor`, `TokenBudget` | 5 层渐进式上下文压缩管道、Token 预算管理 |
| `quality` | 质量门禁上下文 | `QualityGate`, `GateCheck` | Spec 驱动质量门禁，集成于 Session 生命周期，多维度质量检查 |
| `knowledge` | 知识库上下文 | `KnowledgeProvider`, `RulesRegistry` | 知识库集成（Rules / Skills / 企业知识），注入 Prompt 构建管道 |
...
| `tools/hook` | 工具生命周期上下文 | `ToolHook` trait, `HookRegistry`, `ToolEvent`（8 种事件） | 钩子系统：工具执行前后事件拦截（PreCommand / PostCommand） |
| `tools/plugin` | 插件上下文 | `Plugin` trait, `PluginManager`, `McpConnector` | 动态工具加载、MCP 连接器抽象、Plugin collect_commands() |
| `tools/command` | 命令上下文 | `Command`, `CommandRegistry`, `/help`, `/status` | 斜杠命令系统 |
...
- `code-agent tool new` — 自定义工具脚手架命令，快速创建符合 Tool trait 的工具模板（详见 [自定义工具SDK.md](../doc/自定义工具SDK.md)）
...
| 协作层 | 团队协作审计（audit_log 表）：分享/修改/撤销全量记录 | core::agent | — |

---

## 自定义工具 SDK

定制码匠支持通过 `code-agent tool new` CLI 命令快速创建自定义工具：

```bash
# 创建新的自定义工具项目
code-agent tool new my-tool --description "我的自定义工具"

# 生成的工具模板包含完整的 Tool trait 实现骨架
# 开发者只需填充 execute() 方法即可
```

### 特性
- **CLI 脚手架**：`code-agent tool new` 一键生成工具模板，包含 Tool trait 实现、单元测试、输入 schema 定义
- **零配置注册**：工具编译为动态库后，自动发现并注册到 ToolRegistry
- **完整开发文档**：见 [自定义工具SDK.md](../doc/自定义工具SDK.md)
- **示例项目**：`examples/web-search/` 和 `examples/web-fetch/` 提供完整参考实现

### 工具生命周期
1. `code-agent tool new <name>` — 生成工具骨架
2. 实现 `Tool` trait 的 `execute()` 方法
3. 编写单元测试验证工具行为
4. 编译并放置到工具目录，ToolRegistry 自动加载
