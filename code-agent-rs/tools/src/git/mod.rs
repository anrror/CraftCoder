    ...
    pub(crate) fn current_branch_name(&self) -> GitResult<String> {
        ...
        match self.repo.head() {
            ...
            Err(e) if e.code() == git2::ErrorCode::UnbornBranch || e.code() == git2::ErrorCode::NotFound => {
                // Unborn branch or HEAD not found: freshly initialized repo with no commits.
                // Extract the branch name from the HEAD symbolic ref or the filesystem.
                let branch = self
                    .repo
                    .find_reference("HEAD")
                    .ok()
                    ...
                    .or_else(|| {
                        // Fallback: read .git/HEAD directly from the filesystem.
