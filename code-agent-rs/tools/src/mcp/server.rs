...
impl McpServer {
    ...
    fn handle_tools_call(&self, request: &JsonRpcRequest) -> JsonRpcResponse {
        ...
        // Look up and execute the tool via the registry
        // Note: tool execution is async, but handle_* methods are sync.
        // We spawn a blocking call for simplicity.
        let tool = match self.registry.get(tool_name) {
            Some(t) => t,
            None => {
                // Create the result inline since we can't easily return async here
        ...
        // Execute synchronously via tokio runtime
        let result = self.execute_tool_blocking(tool, tool_name, arguments);
    ...
    /// 通过注册表执行工具（阻塞等待异步结果）
    ///
    /// 【领域含义】在同步上下文中通过 tokio runtime 阻塞执行异步工具调用。
    /// 【核心职责】获取当前 tokio runtime → `block_on` 执行工具 → 返回 `McpToolResult`。
    fn execute_tool_blocking(
        &self,
        tool: Arc<dyn Tool>,
        _tool_name: &str,
        arguments: serde_json::Value,
    ) -> McpServerResult<McpToolResult> {
        // Use the current tokio runtime to block on the async execution
        let rt = tokio::runtime::Handle::try_current().map_err(|_| {
            McpServerError::ExecutionError("no tokio runtime available".into())
        })?;

        let result = rt.block_on(async { tool.execute(arguments).await });
...
mod tests {
    ...
    async fn tools_call_executes_tool_and_returns_result() {
        ...
        // Call the glob tool (needs current_thread runtime for block_on)
