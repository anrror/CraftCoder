...
impl ExecCli {
    ...
    /// v2: Construct a ToolRouter wrapping the resolved registry.
    fn resolve_tool_router(&self) -> Option<Arc<ToolRouter>> {
        let registry = self.resolve_tool_registry();
        let permission = Arc::new(PermissionEnforcer::new(
            code_agent_protocol::PermissionMode::Auto,
        ));
        Some(Arc::new(ToolRouter::new(registry, permission)))
    }
}
...
pub fn create_default_tool_registry(
    ...
    cmd_registry: Option<&mut CommandRegistry>,
) -> DefaultToolRegistry {
    let mut registry = DefaultToolRegistry::new();
    ...
    if !mcp_configs.is_empty() {
        register_mcp_plugins_from_configs(mcp_configs, &mut registry, cmd_registry);
    }
...
/// 若提供 `cmd_registry`，插件注册的命令会被收集到其中。
///
/// 【核心职责】遍历配置 → 过滤无效 → 创建 McpPlugin → load_all → collect_commands。
fn register_mcp_plugins_from_configs(
    ...
    rt.block_on(async {
        ...
        // v2: Collect plugin-provided commands into the registry
        if let Some(cr) = cmd_registry {
            pm.collect_commands(cr);
        }
    });
}
...
impl ExecCli {
    ...
    pub async fn execute(&self) -> Result<ExecExitCode, ExecError> {
        ...
        let tool_router = self.resolve_tool_router();
        ...
        let session_config = SessionConfigBuilder::default()
            ...
            .tool_router(tool_router.expect("tool_router should be resolved"))
            .build();
