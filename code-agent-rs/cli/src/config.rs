...
// ---------------------------------------------------------------------------
// QualityConfig — 质量门禁配置
// ---------------------------------------------------------------------------

/// 质量门禁配置（可选 TOML 扩展节 `[quality]`）
///
/// 【领域含义】控制 Plan 引擎执行过程中的质量门禁行为。对应 SpecConfig + PlanConfig 的 gate 相关字段。
/// 【核心职责】决定门禁是否启用、哪些规则生效、重试行为等。
...
pub struct QualityConfig {
    ...
    /// 自动重试失败节点（默认 true）
    pub auto_retry: bool,
    /// 重试时是否使用不同的 Coder prompt（默认 false）
...
impl AgentConfig {
    ...
    pub fn finalize(&self) -> Config {
        ...
        Config {
            ...
            plan_enabled: self.plan_enabled.unwrap_or(defaults.plan_enabled),
            quality: self
                .quality
                .clone()
...
impl ConfigBuilder {
    ...
    /// 设置质量门禁配置
    ///
    /// 【领域含义】覆盖质量门禁配置（TOML `[quality]` 节）。
    /// 【核心职责】设置 quality 字段，按字段合并覆盖。
    pub fn quality(mut self, v: QualityConfig) -> Self {
        self.inner.quality = Some(v);
        self
    }

    /// 设置知识模块配置
    ///
    /// 【领域含义】覆盖知识模块配置（TOML `[knowledge]` 节）。
    /// 【核心职责】设置 knowledge 字段。
    pub fn knowledge(mut self, v: KnowledgeConfig) -> Self {
        self.inner.knowledge = Some(v);
        self
    }
    ...
    pub fn merge(mut self, other: &AgentConfig) -> Self {
        ...
        if let Some(v) = &other.quality {
            self.inner.quality = Some(v.clone());
        }
