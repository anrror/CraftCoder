...
enum Command {
    ...
    /// Scaffold and manage custom tools.
    #[command(subcommand)]
    Tool(ToolCmd),
...
/// Commands for managing custom tools.
#[derive(Subcommand)]
enum ToolCmd {
    /// Scaffold a new custom tool project.
    New {
        /// Name of the tool (snake_case, e.g. "web_search")
        name: String,
...
use code_agent_core::model::{create_model_client, ModelClient, ModelConfig, ProviderKind};
    ...
    let tool_registry: Arc<dyn ToolRegistry> = Arc::new(create_default_tool_registry(&config.mcp_servers, None));
...
fn cmd_tool(cmd: ToolCmd) -> i32 {
    match cmd {
        ToolCmd::New { name, output } => {
            let output_dir = std::path::PathBuf::from(&output);
            match code_agent_cli::scaffold::generate_tool_project(&name, &output_dir) {
                Ok(project_dir) => {
                    println!("Tool project generated at {}", project_dir.display());
                    println!();
                    println!("Next steps:");
                    println!("  cd {}", project_dir.display());
                    println!("  cargo build --release");
                    println!("  # Then add to config.toml to load into CraftCoder");
                    0
                }
                Err(e) => {
                    eprintln!("error: {e}");
