...
async fn cmd_tui(_config: Config, initial_prompt: Option<String>) -> i32 {
    ...
    if let Some(prompt) = initial_prompt {
        ...
        // In a full integration, the prompt would be sent via the agent channel.
        eprintln!(
            "note: initial prompt '{}' set; full agent integration required to process",
            prompt
        );
    }
