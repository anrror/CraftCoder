...
impl InputBar {
    ...
    /// 检测斜杠命令
    ///
    /// 【领域含义】检测输入文本是否以斜杠命令开头。用于命令高亮渲染。
    /// 【核心职责】识别已知的斜杠命令前缀。
    pub fn detect_command(text: &str) -> Option<&'static str> {
        let text = text.trim();
        if !text.starts_with('/') {
            return None;
        }
        ...
        // Built-in commands — kept in sync with CommandRegistry defaults
        match command {
            "/help" => Some("/help"),
            "/status" => Some("/status"),
            "/plan" => Some("/plan"),
            ...
            "/review" => Some("/review"),
            "/search" => Some("/search"),
            _ => None, // Agent will handle unrecognized commands as user input
        }
    }
