...
use code_agent_core::FeedbackSink;
use code_agent_core::tools::command::CommandRegistry;

// ---------------------------------------------------------------------------
// PlanState — TUI-level tracking for the /plan workflow
// ---------------------------------------------------------------------------
...
pub struct App {
    ...
    /// Sender for user messages to the agent engine.
    agent_tx: mpsc::UnboundedSender<String>,

    /// Sender for /plan command descriptions (routed to plan background task).
    plan_tx: mpsc::UnboundedSender<String>,

    /// Sender for plan approval decisions (true=approve, false=reject).
    approval_tx: mpsc::UnboundedSender<bool>,

    /// Current plan workflow state.
    ...
    /// Command registry for /-style commands.
    command_registry: CommandRegistry,

    /// Feedback sink for persisting user ratings.
...
impl App {
    ...
    ///
    /// # 参数
    /// * `agent_tx` — 用户消息发送端，TUI 通过此通道将用户输入发送给 Agent 引擎。
    /// * `agent_rx` — Agent ResponseEvent 的接收端，TUI 从此通道读取流式响应和工具调用。
    /// * `plan_tx` — /plan 命令描述发送端，发送给 plan 后台任务。
    /// * `approval_tx` — plan 审批结果发送端。
    ///
    /// # 错误
    ...
    pub async fn run(&mut self) -> io::Result<()> {
        ...
        while self.running {
            ...
            match event {
                ...
                // Plan events are handled in mod.rs directly
                AppEvent::PlanDecomposed { .. }
                | AppEvent::PlanProgress(..)
                | AppEvent::PlanApprovalRequired { .. }
                | AppEvent::PlanCompleted { .. }
                | AppEvent::PlanError(..) => {
                    self.handle_plan_event(event);
                }
            }
        ...
        if self.feedback.is_active() && self.feedback.handle_key(key) {
            ...
            if self.feedback.state == feedback::FeedbackPromptState::Submitted {
                if let Some(event) = self.feedback.build_event() {
                    // Store feedback via the sink
                    if let Err(e) = self.feedback_sink.store(&event) {
                        tracing::warn!(error = %e, "Failed to store feedback");
                    }
                    self.chat.push_message(chat::ChatMessage {
                        role: chat::ChatRole::System,
                        content: format!(
                            "Feedback submitted: {}",
                            if event.rating == code_agent_core::Rating::ThumbsUp {
                                "👍 Thumbs Up"
                            } else {
                                "👎 Thumbs Down"
                            }
            ...
            KeyCode::Enter => {
                ...
                match action {
                    InputAction::Submit(text) => {
                        ...
                        if self.plan_state == PlanState::AwaitingApproval {
                            let approved = text == "y" || text == "yes" || text == "Y";
                            if approved {
                                self.chat.push_message(chat::ChatMessage {
                            ...
                            self.plan_state = if approved { PlanState::Executing } else { PlanState::Inactive };
                            let _ = self.approval_tx.send(approved);
                            self.chat.scroll_to_bottom();
                            return;
                        }

                        // ── Executing: ignore all input ─────────────
                        if self.plan_state == PlanState::Executing {
                            self.chat.push_message(chat::ChatMessage {
                                role: chat::ChatRole::System,
                                content: format!("Plan is executing — ignoring input: {}", text),
                                is_streaming: false,
                            });
                            self.chat.scroll_to_bottom();
                            return;
                        ...
                        if text.starts_with('/') {
                            ...
                            let trimmed = cmd_name.trim_start_matches('/');

                            // /plan has special TUI integration (plan_tx channels)
                            if trimmed == "plan" {
                                let desc = text["/plan".len()..].trim().to_string();
                                if desc.is_empty() {
                                    self.chat.push_message(chat::ChatMessage {
                                        role: chat::ChatRole::Error,
                                        content: "Usage: /plan <task description>".into(),
                                        is_streaming: false,
                                    });
                                } else {
                                    self.chat.push_message(chat::ChatMessage {
                            ...
                            let result = self.command_registry.execute(&text, Default::default()).await;

                            // Inject real session state into /status output
                            let output = if trimmed == "status" {
                                let total_tokens = self.status.prompt_tokens + self.status.completion_tokens;
                                let descriptions: Vec<String> = self.command_registry.list_descriptions()
                                    .into_iter()
                                ...
                                format!(
                                    "Session Status\n\
                                     ...
                                     Input Mode:   {mode}\n\
                                     \n\
                                     (use `/plan <task>` to start a session)\n\
                                     \n\
                                     Loaded Commands ({n}):\n\
                                     ──────────────────────\n\
                                     {descs}",
                                    session = if self.status.session_id.is_empty() { "—" } else { &self.status.session_id },
                                    model = if self.status.model_name.is_empty() { "—" } else { &self.status.model_name },
                                    conn = self.status.connection.label(),
                                    turns = self.status.turn_count,
                                    total = total_tokens,
                                    prompt = self.status.prompt_tokens,
                                    completion = self.status.completion_tokens,
                                    mode = self.status.mode.label(),
                                    descs = descriptions.join("\n"),
                                    n = n,
                                )
                            } else {
                                result.output
                            ...
                            self.chat.push_message(chat::ChatMessage {
                                role: if result.success { chat::ChatRole::System } else { chat::ChatRole::Error },
                                content: output,
                                is_streaming: false,
                            });
                            self.chat.scroll_to_bottom();
                        ...
                        // Send the user message to the agent engine via the channel
                        let _ = self.agent_tx.send(text.clone());
                        self.chat.scroll_to_bottom();
                    }
                    InputAction::None => {}
                    _ => {}
    ...
    /// Handle a plan-related event (decomposed, progress, approval, completed).
    fn handle_plan_event(&mut self, event: AppEvent) {
        match event {
            AppEvent::PlanDecomposed { plan_name, phase_count, node_count, phases } => {
                self.chat.push_message(chat::ChatMessage {
                    role: chat::ChatRole::PlanPhase,
                    content: format!("📋 Plan: {} — {} nodes in {} phases", plan_name, node_count, phase_count),
                    is_streaming: false,
                });
                for phase in &phases {
                    self.chat.push_message(chat::ChatMessage {
            ...
            AppEvent::PlanProgress(progress) => {
                match progress {
                    code_agent_core::agent::plan::PlanProgressEvent::PhaseChanged { phase, completed, total } => {
                        self.chat.push_message(chat::ChatMessage {
                            role: chat::ChatRole::PlanPhase,
                            content: format!("▶ Phase: {} [{}/{}]", phase, completed, total),
                            is_streaming: false,
                        });
                    }
                    code_agent_core::agent::plan::PlanProgressEvent::NodeStarted { node_id, label, .. } => {
                        self.chat.push_message(chat::ChatMessage {
                            role: chat::ChatRole::PlanNode,
                            content: format!("  ▶ {} ({})", label, node_id),
                            is_streaming: true,
                        });
                    }
                    code_agent_core::agent::plan::PlanProgressEvent::NodeCompleted { node_id, label, .. } => {
                    ...
                    code_agent_core::agent::plan::PlanProgressEvent::NodeFailed { node_id, label, error, .. } => {
                        self.chat.finalize_streaming();
                        self.chat.push_message(chat::ChatMessage {
                            role: chat::ChatRole::Error,
                            content: format!("  ✗ {} ({}): {}", label, node_id, error),
                            is_streaming: false,
                        });
                    }
                    code_agent_core::agent::plan::PlanProgressEvent::NodeCancelled { node_id, label, .. } => {
                        self.chat.finalize_streaming();
                        self.chat.push_message(chat::ChatMessage {
                            role: chat::ChatRole::PlanNode,
                            content: format!("  – {} ({}) — cancelled", label, node_id),
                            is_streaming: false,
                        });
                    }
                    code_agent_core::agent::plan::PlanProgressEvent::PlanCompleted { success, .. } => {
                        self.chat.finalize_streaming();
                        self.chat.push_message(chat::ChatMessage {
                            role: chat::ChatRole::System,
                            content: if success {
                                "✓ Plan execution completed successfully".into()
                            } else {
                                "✗ Plan completed with failures".into()
                            },
                            is_streaming: false,
                        });
                    }
            ...
            AppEvent::PlanApprovalRequired { plan_name, phase_count, node_count } => {
                self.plan_state = PlanState::AwaitingApproval;
                self.chat.push_message(chat::ChatMessage {
                    role: chat::ChatRole::System,
                    content: format!("Approve plan '{}' ({} phases, {} nodes)? Type 'y' to execute, or anything else to cancel.", plan_name, phase_count, node_count),
                    is_streaming: false,
                });
            }
            AppEvent::PlanCompleted { success, summary } => {
                self.plan_state = PlanState::Inactive;
                self.chat.push_message(chat::ChatMessage {
                    role: chat::ChatRole::System,
                    content: format!("[Plan {}] {}", if success { "SUCCESS" } else { "FAILED" }, summary),
                    is_streaming: false,
                });
            }
            AppEvent::PlanError(err) => {
                self.plan_state = PlanState::Inactive;
                self.chat.push_message(chat::ChatMessage {
                    role: chat::ChatRole::Error,
                    content: format!("Plan error: {}", err),
                    is_streaming: false,
                });
            }
            _ => {}
