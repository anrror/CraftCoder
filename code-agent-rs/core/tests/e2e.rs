...
async fn e2e_coding_task_text_only() {
    ...
    let mut session = Session::new(make_config("e2e-coding-text", responses, ToolRegistry::new())).await;
    let events = session.run_turn(TurnInput {
        thread_id: ThreadId::from("t1"),
...
async fn e2e_debug_unknown_tool() {
    ...
    let mut session = Session::new(make_config("e2e-debug-unknown", responses, ToolRegistry::new())).await;
    let events = session.run_turn(TurnInput {
        thread_id: ThreadId::from("t1"),
...
async fn e2e_benchmark_startup() {
    ...
    let session = Session::new(make_config("bench-startup", vec![], ToolRegistry::new())).await;
...
async fn e2e_benchmark_multi_turn_throughput() {
    ...
    let mut session = Session::new(make_config("bench-throughput", all_responses, ToolRegistry::new())).await;
    let start = std::time::Instant::now();
    for i in 0..10 {
        let events = session.run_turn(TurnInput {
            thread_id: ThreadId::from("t1"),
