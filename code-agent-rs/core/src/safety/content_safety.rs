...
/// 返回 	rue 表示存在至少一个注入模式匹配。
///
/// Quick check for injection patterns using compiled regex.
///
/// Returns 	rue if any injection pattern matches the content.
...
impl ContentSafetyLayer {
    ...
    /// # Arguments
    ///
    /// * 	ool_name — The name of the tool that produced the content.
