...
/// Removes conversation history beyond the preserved turn count.
///
/// Messages older than preserve_last_turns turns are removed from the
/// beginning of the history, keeping only the most recent turns intact.
///
...
impl CompactionLayer for Snip {
    ...
    fn compact(&self, context: &mut ContextState) -> CompactionResult {
        let tokens_before = estimate_total_tokens(&context.messages);
        let preserve = context.config.preserve_last_turns;

        if preserve == 0 || context.messages.is_empty() {
        ...
        if turn_starts.len() <= preserve {
            return CompactionResult {
                layer: self.name().to_string(),
                tokens_before,
                tokens_after: tokens_before,
                messages_removed: 0,
                // Still signal downstream if ratio is high enough for other layers
                needs_more: context.token_ratio() >= context.config.compress_threshold,
            };
        }

        // Remove messages before the preserve-th-last turn
        let keep_from = turn_starts[turn_starts.len() - preserve];
        let removed_count = keep_from;
        let removed: Vec<Message> = context.messages.drain(0..keep_from).collect();
        let tokens_removed = estimate_total_tokens(&removed);
        let tokens_after = tokens_before.saturating_sub(tokens_removed);

        debug!(
            layer = self.name(),
            removed = removed_count,
            preserved_turns = preserve,
            before = tokens_before,
            after = tokens_after,
            "Snip removed deep history"
        );
...
/// LLM-based semantic summarization placeholder.
///
/// This layer is invoked under extreme cache pressure (95%+). In production,
/// it would call a lightweight LLM to produce a semantic summary of the
/// conversation. Currently, it falls back to a deterministic extraction of
    ...
    fn compact(&self, context: &mut ContextState) -> CompactionResult {
        ...
        if context.messages.is_empty() || self.keep_turns == 0 {
            return CompactionResult {
                ...
                tokens_after: tokens_before,
                messages_removed: 0,
                // AutoCompact is the last layer     report if ratio still above evict threshold
                needs_more: context.token_ratio() >= context.config.evict_threshold,
            };
        }
        ...
        let semantic_summary = build_semantic_summary(&to_summarize, &context.system_instructions);
        context.messages.insert(0, Message::UserMessage {
...
mod tests {
    ...
    fn snip_removes_history_beyond_preserved_turns() {
        ...
        assert_eq!(ctx.messages.len(), 6); // 3 preserved turns * 2 msg per turn
        // First message should be from turn 8 (10 - 3 + 1)
        if let Message::UserMessage { ref content } = ctx.messages[0] {
            assert!(content.contains("Request 8"), "should keep turn 8, got: {}", content);
        } else {
    ...
    fn snip_preserves_exact_turn_count() {
        ...
        // 3 turns preserved = 6 messages
        assert_eq!(ctx.messages.len(), 6);

        // Last message should be from turn 5
        if let Message::AssistantMessage { ref content } = ctx.messages[5] {
    ...
    fn each_layer_triggers_independently() {
        ...
        assert!(
            layer_names.contains(&"Snip"),
            "Snip should trigger. Layers: {:?}", layer_names
        );
        assert!(
            layer_names.contains(&"Microcompact"),
