...
impl ContextManager {
    ...
    /// Create a ContextManager with an LLM summarizer for AutoCompact.
    pub fn with_summarizer(
        system_instructions: String,
        summarizer: Arc<dyn compactor::SemanticSummarizer>,
    ) -> Self {
        Self::with_config_and_summarizer(
            system_instructions,
    ...
    /// Create a ContextManager with custom config and an LLM summarizer.
    pub fn with_config_and_summarizer(
        system_instructions: String,
        config: ContextConfig,
        summarizer: Arc<dyn compactor::SemanticSummarizer>,
    ) -> Self {
        use compactor::{
            AutoCompact, BudgetReduction, CompactionLayer, CompactionPipeline,
            ContextCollapse, Microcompact, SlidingSnip,
        };
        let layers: Vec<Box<dyn CompactionLayer>> = vec![
            ...
            Box::new(AutoCompact::with_summarizer(config.preserve_last_turns, summarizer)),
        ];
        let pipeline = CompactionPipeline::with_layers(layers);
        Self {
            config: config.clone(),
            current_tokens: 0,
            compaction_count: 0,
