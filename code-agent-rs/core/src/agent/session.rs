...
use crate::tools::hook::{HookRegistry, ToolEvent};
...
use crate::agent::plan::quality::{GateVerdict, QualityGate};
use crate::agent::plan::types::{PlanConfig, PlanNode};
use crate::agent::sub_agent::{AgentResult, AgentStatus};
use crate::agent::KnowledgeProvider;
...
pub struct SessionConfig {
    ...
    /// v2: 可选的工具路由器（带权限执行和钩子分发）。
    /// 当设置后，`execute_tool` 会通过 ToolRouter 路由，
    /// 从而启用 PermissionEnforcer 检查和 PreExecute/PostExecute 钩子。
    pub tool_router: Option<Arc<ToolRouter>>,

    /// v2: 可选的钩子注册表（生命周期事件分发）。
    /// 当设置后，SessionCreate、SessionDestroy、PreModelCall、PostModelCall
    /// 事件会在对应生命周期点分发。
    ...
    /// v2: 可选的知识提供者（Phase E 知识体系）。
    /// 当设置后，`Session::new()` 会调用 `format_context()` 并将
    /// 结果注入到 system_instructions 中，使 Rules/Skills 知识
    /// 在每次模型调用时生效。
    pub knowledge: Option<Arc<dyn KnowledgeProvider>>,

    /// v2: 可选的 QualityGate（工具级质量门禁）。
    /// 当设置后，每次工具执行完成后会通过门禁检查，
    /// 根据裁决结果决定是否重试或跳过上下文注入。
    #[allow(dead_code)]
    pub quality_gate: Option<Arc<dyn QualityGate>>,
}
...
impl SessionConfigBuilder {
    ...
    /// v2: Set the tool router (optional).
    /// When set, `execute_tool` routes through ToolRouter for permission enforcement
    /// and PreExecute/PostExecute hook dispatch.
    pub fn tool_router(mut self, router: Arc<ToolRouter>) -> Self {
        self.tool_router = Some(router);
        self
    }

    /// v2: Set the hook registry (optional).
    /// When set, lifecycle hooks (SessionCreate, SessionDestroy, PreModelCall,
    /// PostModelCall) are dispatched at the appropriate points.
    ...
    /// v2: Set the knowledge provider (optional).
    /// When set, `Session::new()` calls `format_context()` and prepends the
    /// result to system_instructions, injecting Rules/Skills into every model call.
    pub fn knowledge(mut self, knowledge: Arc<dyn KnowledgeProvider>) -> Self {
        self.knowledge = Some(knowledge);
        self
    }

    /// v2: Set the quality gate (optional).
    /// When set, tool execution results are checked against the gate
    /// after each tool call, with Retry verdict triggering re-execution
    /// (up to 2 attempts) and Fail verdict skipping context injection.
    pub fn quality_gate(mut self, gate: Arc<dyn QualityGate>) -> Self {
...
pub struct Session {
    ...
    /// v2: 工具路由器（可选，带权限执行和钩子分发）
    #[allow(dead_code)]
    tool_router: Option<Arc<ToolRouter>>,

    /// v2: 钩子注册表（可选，生命周期事件分发）
    #[allow(dead_code)]
    hook_registry: Option<Arc<HookRegistry>>,

    /// v2: 知识提供者（可选，Phase E 知识体系）
    #[allow(dead_code)]
    knowledge: Option<Arc<dyn KnowledgeProvider>>,
...
impl Session {
    ...
    pub async fn new(config: SessionConfig) -> Self {
        // v2: If knowledge provider is configured, inject knowledge context
        // into system_instructions before initialising the context manager.
        let system_instructions = if let Some(ref kp) = config.knowledge {
            let knowledge_ctx = kp.format_context(&config.system_instructions).await;
            if knowledge_ctx.is_empty() {
                config.system_instructions.clone()
            } else {
                format!("{}\n{}", knowledge_ctx, config.system_instructions)
            }
        } else {
            config.system_instructions.clone()
        };
        ...
        let context_manager = match config.max_context_tokens {
            ...
            None => crate::context::ContextManager::new(system_instructions),
        };
        ...
        let tool_router = config.tool_router.clone();
        let hook_registry = config.hook_registry.clone();
        let knowledge = config.knowledge.clone();
        let quality_gate = config.quality_gate.clone();

        // v2: Dispatch SessionCreate lifecycle hook
        if let Some(ref hr) = hook_registry {
            hr.dispatch(&ToolEvent::SessionCreate {
                thread_id: String::new(),
                session_id: config.id.0.clone(),
    ...
    async fn execute_tool(&self, tc: &ToolCall) -> Result<ToolResultMessage, ToolResultMessage> {
        // v2: Route through ToolRouter when available (enables PermissionEnforcer + hooks)
        if let Some(ref router) = self.tool_router {
            let result = router.route(tc).await;
            return if result.error.is_some() { Err(result) } else { Ok(result) };
        }

        // Legacy path: direct registry access (backward compatible)
...
///
/// v2: When a quality_gate is configured on the session, each tool execution
/// result is checked against the gate. A Retry verdict triggers re-execution
/// (up to 2 attempts), a Fail verdict skips context injection for that tool.
///
...
impl SessionStep for ToolExecutionStep {
    async fn execute(&self, session: &mut Session, ctx: &mut StepContext<'_>) -> StepOutcome {
        ...
        for tc in &ctx.tool_calls {
            ...
            // v2: Quality gate retry loop (max 2 retries)
            const MAX_GATE_RETRIES: u32 = 2;
            let mut gate_retry_count: u32 = 0;

            let _trm: ToolResultMessage = 'gate_retry: loop {
                // Execute tool
                let result = session.execute_tool(tc).await;

                // Phase G: capture tool errors for flywheel analysis (every attempt)
                ...
                // v2: Quality gate check (if configured)
                if let Some(ref gate) = session.quality_gate {
                    let (trm_ref, is_err) = match &result {
                        Ok(trm) => (trm, false),
                        Err(trm) => (trm, true),
                    };
                    let agent_result = AgentResult {
                        agent_id: format!("session-{}", tc.name),
                        status: if is_err {
                            AgentStatus::Failed
                        } else {
                        ...
                        events: vec![],
                        final_message: trm_ref.output.clone(),
                        error: trm_ref.error.clone(),
                    };
                    ...
                    let verdict = gate
                        .check(&agent_result, &plan_node, &PlanConfig::default())
                        .await;
                    ...
                    match verdict {
                        ...
                        GateVerdict::Retry { ref reason }
                            if gate_retry_count < MAX_GATE_RETRIES =>
                        {
                            tracing::warn!(
                                tool = %tc.name,
                                attempt = gate_retry_count,
                                %reason,
                                "Quality gate: retrying tool execution"
                            );
                            gate_retry_count += 1;
                            continue 'gate_retry;
                        }
                        GateVerdict::Retry { ref reason } => {
                            tracing::warn!(
                                tool = %tc.name,
                                attempts = gate_retry_count,
                                %reason,
                                "Quality gate: max retries exhausted, proceeding"
                            );
                            // Fall through to normal handling
                        }
                        GateVerdict::Fail { ref reason } => {
                            tracing::warn!(
                                tool = %tc.name,
                                %reason,
                                "Quality gate: tool execution failed gate check, skipping context injection"
                            );
                            ctx.events.push(ResponseEvent::Error {
                                message: format!(
                                    "Tool '{}' failed quality gate: {}",
                                    tc.name, reason
                                ),
                            });
                            // Emit ToolCallEnd with result but skip context injection
                            ...
                            break 'gate_retry trm;
                        }
                    }
                }

                // === Normal flow (Pass, no gate, or retries exhausted) ===
                ...
                session
                    .context_manager
                    .add_message(Message::ToolResult(trm.clone()));
                break 'gate_retry trm;
            };
        }
