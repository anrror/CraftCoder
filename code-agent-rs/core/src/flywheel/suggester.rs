...
impl ImprovementSuggester {
    ...
    fn suggest_for_cluster(&self, cluster: &FailureCluster) -> Vec<Suggestion> {
        ...
        match error_type {
            ...
            "plan_node_failed" => suggestions.push(self.make_suggestion(
                &format!(
                    "Plan node failed in '{}'. Review node dependencies, error handling, \
                     and retry configuration. Consider adding pre-execution validation.",
                    tool_name
                ),
                tool_name,
                cluster.count,
            ...
            "plan_node_cancelled" => suggestions.push(self.make_suggestion(
                &format!(
                    "Plan node cancelled in '{}'. Investigate dependency chain — a parent \
                     node failure may have cascaded. Consider adding graceful degradation.",
                    tool_name
                ),
                tool_name,
                cluster.count,
